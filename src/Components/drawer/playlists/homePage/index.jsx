...
const WelcomePage = () => {
  const topicsArray = topics.map((topic) => (
    <Link
      key={topic.id}
      to={`/playlists/topic/${topic.id}`}
      style={{ textDecoration: 'none', color: 'black' }}
    >
      <div key={topic.id} className="pointer ma0 br4 mb3 b--purple ba pa3 hover-background-color">
        <Body2 className="fr gray">{topic.date}</Body2>
        <Body1 className="">{topic.name}</Body1>
        <Body1 className="mid-gray">{topic.description}</Body1>
        <Body2 className="gray">{topic.author}</Body2>
      </div>
    </Link>
  ));
