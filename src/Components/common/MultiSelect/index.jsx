...
const MultiSelect = ({ options, selectedOptions = [], updateSelectedOptions }) => {
  ...
  const removeOption = useCallback(
    (option) => {
      ...
      setMappedSelectedOptions(
        selectedOptionsInternal.current.map((value) => (
  ...
  const addOption = useCallback(
    (option) => {
      if (
        !selectedOptionsInternal.current.some(
      ...
      const mappedOptions = optionsRecieved.map((option, index) => {
        if ((index + 1) % 2 === 0) {
          return (
            <div
              role="presentation"
              key={option.value}
              className="bg-washed-green pa2 pointer"
        ...
        return (
          <div
            role="presentation"
            key={option.value}
            className="bg-lightest-blue pa2 pointer"
            onClick={() => addOption(option)}
          >
            {option.label}
          </div>
  ...
  return (
    <Container>
      ...
      <TextField label="Select the admin for the contest" className="w-100" outlined>
        <Input value={search} id="adminName" onChange={(e) => onSearchChange(e)} />
      </TextField>
      <div style={{ maxHeight: '100px', overflow: 'scroll' }}>{mappedSearchResults}</div>
    </Container>
  );
};
