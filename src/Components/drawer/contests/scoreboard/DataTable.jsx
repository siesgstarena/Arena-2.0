  ...
  const DataArray = Data.map(entry => {
    return (
      <tr key={entry.id} style={{ fontSize: ".9em" }}>
        <td className="tc pa3">
          <Link className="no-underline dim blue pointer" to={`submission/${entry.id}`}>
          {entry.id}
          </Link>
        </td>
        <td className="tc pa3">
          <Link className="no-underline dim blue pointer" to={`/profile/${entry.who.id}`}>
          {entry.who.username}
          </Link>
        </td>
        ...
        {entry.problem.map(prob => {
          return <td className="tc pa3">{prob.score}</td>;
        })}
      </tr>
    );
  });
